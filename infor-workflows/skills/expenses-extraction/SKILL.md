...
- Template location: !`REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null); find "$REPO_ROOT/templates" "$HOME" -name "INFOR Expense Report Template.xlsx" 2>/dev/null | head -1`
...
find "$REPO_ROOT/templates" "$HOME" -name "INFOR Expense Report Template.xlsx" 2>/dev/null | head -1
```
...
```bash
REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
TEMPLATE=$(find "$REPO_ROOT/templates" "$HOME" -name "INFOR Expense Report Template.xlsx" 2>/dev/null | head -1)
